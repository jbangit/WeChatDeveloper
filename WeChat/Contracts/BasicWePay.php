...
class BasicWePay
{
     ...
     * @throws \WeChat\Exceptions\LocalCacheException
     */
    protected function callPostApi($url, array $data, $isCert = false, $signType = 'HMAC-SHA256', $needSignType = true, $notNeedKey = [])
    {
        $option = [];
        if ($isCert) {
		...
		if(!empty($notNeedKey)) {
			for($i = 0; $i < count($notNeedKey); $i++) {
				unset($params[$notNeedKey[$i]]);
			}
		}
